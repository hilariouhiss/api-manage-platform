...
pub struct ServerConfig {
    ...
    #[validate(range(min = 1, max = 65535, message = "server.port must be between 1 and 65535"))]
    pub port: u16,
}
...
pub struct DatabaseConfig {
    ...
    #[validate(range(min = 1, max = 100, message = "database.max_connections must be between 1 and 100"))]
...
pub struct JwtConfig {
    ...
    #[validate(range(min = 1, max = 720, message = "jwt.expiry_hours must be between 1 and 720"))]
...
mod tests {
    ...
    fn test_server_config_empty_host_invalid() {
        let cfg = ServerConfig { host: "".into(), port: 3000 };
        assert!(cfg.validate().is_err());
    }
    ...
    fn test_database_config_valid() {
        let cfg = DatabaseConfig { url: "postgres://localhost/db".into(), max_connections: 10 };
        assert!(cfg.validate().is_ok());
    }

    #[test]
    fn test_database_config_max_connections_zero_invalid() {
        let cfg = DatabaseConfig { url: "postgres://localhost/db".into(), max_connections: 0 };
        assert!(cfg.validate().is_err());
    }

    #[test]
    fn test_database_config_empty_url_invalid() {
        let cfg = DatabaseConfig { url: "".into(), max_connections: 10 };
        assert!(cfg.validate().is_err());
    }
    ...
    fn test_valkey_config_valid() {
        let cfg = ValkeyConfig { url: "redis://localhost:6379".into() };
        assert!(cfg.validate().is_ok());
    }
    ...
    fn test_jwt_config_valid() {
        let cfg = JwtConfig { secret: "a-strong-secret-at-least-32-chars!".into(), expiry_hours: 24 };
        assert!(cfg.validate().is_ok());
    }

    #[test]
    fn test_jwt_config_secret_too_short_invalid() {
        let cfg = JwtConfig { secret: "short".into(), expiry_hours: 24 };
        assert!(cfg.validate().is_err());
    }

    #[test]
    fn test_jwt_config_secret_empty_invalid() {
        let cfg = JwtConfig { secret: "".into(), expiry_hours: 24 };
        assert!(cfg.validate().is_err());
    }

    #[test]
    fn test_jwt_config_expiry_zero_invalid() {
        let cfg = JwtConfig { secret: "a-strong-secret-at-least-32-chars!".into(), expiry_hours: 0 };
        assert!(cfg.validate().is_err());
    }

    #[test]
    fn test_jwt_config_expiry_above_720_invalid() {
        let cfg = JwtConfig { secret: "a-strong-secret-at-least-32-chars!".into(), expiry_hours: 721 };
        assert!(cfg.validate().is_err());
    }
    ...
    fn test_logging_config_valid_json() {
        let cfg = LoggingConfig { level: "info".into(), format: "json".into() };
        assert!(cfg.validate().is_ok());
    }

    #[test]
    fn test_logging_config_valid_pretty() {
        let cfg = LoggingConfig { level: "debug".into(), format: "pretty".into() };
        assert!(cfg.validate().is_ok());
    }

    #[test]
    fn test_logging_config_invalid_level() {
        let cfg = LoggingConfig { level: "verbose".into(), format: "json".into() };
        assert!(cfg.validate().is_err());
    }

    #[test]
    fn test_logging_config_invalid_format() {
        let cfg = LoggingConfig { level: "info".into(), format: "text".into() };
        assert!(cfg.validate().is_err());
    }
    ...
    fn test_app_config_nested_validation_propagates() {
        let cfg = AppConfig {
            server: ServerConfig { host: "0.0.0.0".into(), port: 0 }, // invalid port
            database: DatabaseConfig { url: "postgres://localhost/db".into(), max_connections: 10 },
            valkey: ValkeyConfig { url: "redis://localhost:6379".into() },
            jwt: JwtConfig { secret: "a-strong-secret-at-least-32-chars!".into(), expiry_hours: 24 },
